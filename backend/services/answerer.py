...
- ③그래서 무엇을 할 수 있는가: 수치를 언급하지 말고, 어떤 요인이 있으면 유리하게 작용할
  ...
  그래프 데이터에 respondent_arguments(피신청인이 실제로 폈던 반박과 그 결말) 또는
  evidence_patterns(어떤 자료가 있으면 유리하게 작용했는지)가 들어있으면, 은행이 어떤
  논리로 나올 수 있고 무엇을 준비해두면 좋을지를 1~2문장으로 정성적으로 언급하십시오.
  이때도 배척률·비율 같은 구체적인 수치는 절대 쓰지 마십시오 — 화면 카드("은행은 이렇게
  반박할 수 있습니다", "이런 자료가 있으면 유리해요")에 이미 표시되므로, 본문에서는
  "은행 쪽에서는 ~라는 논리를 들 수 있는데, 이에 대비해 ~를 준비해두시면 좋습니다"
  정도로만 서술하십시오. 두 데이터 모두 비어있으면 이 부분은 자연스럽게 생략하십시오.
